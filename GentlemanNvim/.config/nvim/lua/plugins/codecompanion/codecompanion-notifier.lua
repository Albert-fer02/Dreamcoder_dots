...
local function refresh_notifications(key)
  return function()
    ...
    vim.notify(req.msg, vim.log.levels.INFO, {
      ...
      opts = function(notif)
        local icon = " "
        if not req.done then icon = spinner_frame() end
        notif.icon = icon
      end,
    })
  end
...
local function finished(ev)
  ...
  if data.status == "success" then req.msg = "Done."
  elseif data.status == "error" then req.msg = "Error!"
  else req.msg = "Cancelled." end
