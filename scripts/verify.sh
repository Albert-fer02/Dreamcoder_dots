...
for file in starship.toml starship-light.toml; do STARSHIP_CONFIG="${DREAMCODER_DOTS_DIR}/DreamcoderShell/.config/${file}" starship explain >/dev/null; done
...
for case_args in "${CONTROL_CASES[@]}"; do
  read -r -a argv <<<"${case_args}"
  control "${argv[@]}" >/dev/null
done
