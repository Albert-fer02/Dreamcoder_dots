...
HOUR="$(date +%H)"
EXPECTED="dark"
if ((10#${HOUR} >= LIGHT_START && 10#${HOUR} < DARK_START)); then EXPECTED="light"; fi
...
gtk_mode() {
  v=$(grep -m1 '^gtk-application-prefer-dark-theme=' "${CONFIG_HOME}/gtk-3.0/settings.ini" 2>/dev/null | cut -d= -f2)
...
printf 'time=%s expected=%s window=light:%s-%s\n' "$(date '+%H:%M %Z')" "${EXPECTED}" "${LIGHT_START}" "${DARK_START}"
status=0
