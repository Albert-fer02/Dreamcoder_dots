...
TARGETS = [
    ...
    ("bat", "bat syntax viewer", "Bat/.config/bat/themes/Dreamcoder-Light.tmTheme", "manual: run bat against shell/python/markdown samples"),
    ("delta", "delta git diff", "themes/dreamcoder/delta-dreamcoder-light.gitconfig", "manual: preview git diff with additions/deletions"),
    ("fzf", "fzf picker", "themes/dreamcoder/fzf-dreamcoder-light.sh", "manual: open fzf with preview window"),
    ("btop", "btop monitor", "themes/dreamcoder/btop-dreamcoder-light.theme", "manual: open btop and capture process + graph panels"),
    ("dunst", "Dunst notifications", "themes/dreamcoder/dunst-dreamcoder-light.conf", "manual: trigger low/normal/critical notifications"),
    ("cava", "Cava visualizer", "themes/dreamcoder/cava-dreamcoder-light.config", "manual: capture Cava bars on light terminal"),
    ("obsidian", "Obsidian notes", "themes/dreamcoder/obsidian-dreamcoder-light.css", "manual: capture note, code block, and callout"),
    ("firefox", "Firefox chrome", "themes/dreamcoder/firefox-dreamcoder-light.css", "manual: capture toolbar, tabs, and page chrome"),
]
...
def visual_audit() -> dict[str, Any]:
    ...
    plan = visual_plan()
    sources = [
        {"key": target["key"], "path": target["source"], "exists": (root / target["source"]).exists()}
        for target in plan["targets"]
    ]
    baselines = [
        {"key": target["key"], "path": target["baseline"], "exists": (root / target["baseline"]).exists()}
        for target in plan["targets"]
    ]
...
def visual_audit_markdown(report: dict[str, Any]) -> str:
    ...
    rows.append("| Runtime contracts | " + ("ok" if all(report["checks"]["runtime"].values()) else "attention") + " |")
    return "\n".join([
