...
def waybar_matugen_content(c: dict[str, str]) -> str:
    ...
    lines = [
        ...
        " */",
        f'@define-color blur_background {m["background"].replace("ff)", "4d)")};',
        f'@define-color blur_background8 {m["background"].replace("ff)", "cc)")};',
        "",
    ]
    for key in sorted(m.keys()):
        lines.append(f"@define-color {key} {m[key]};")
    return "\n".join(lines) + "\n"
...
def rofi_matugen_content(c: dict[str, str]) -> str:
    ...
    m = _map_dc_to_material(c)
    lines = [
        "/* Dreamcoder colors for ML4W Rofi — only replaces Matugen color values. */",
        "* {",
    ]
    for key in sorted(m.keys()):
        lines.append(f"    {key}: {m[key]};")
