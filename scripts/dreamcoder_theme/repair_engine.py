...
def safe_action(action_id: str, check: Any, description: str, source: Path | None = None) -> dict[str, Any]:
    action = {
        "id": action_id,
        "check": check.name,
        "safe": True,
...
def repair_plan() -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {"actions": len(actions), "safe": safe_count, "manual": len(actions) - safe_count},
