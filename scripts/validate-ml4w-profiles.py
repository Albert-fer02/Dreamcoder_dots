...
def _validate_binding(
    ...
    if len(mods) > 0 and "SUPER" in mods and mods[0] != "SUPER":
        errors.append(f"  ⚠ {name} binding[{index}]: SUPER should be first in mods array")

    # Bind type validation
    if bind_type not in VALID_BIND_TYPES:
        errors.append(f"  ❌ {name} binding[{index}]: invalid bind_type '{bind_type}'")
    ...
    if submap_entry and submap_entry != submap_entry.lower():
        errors.append(f"  ⚠ {name} binding[{index}]: submap_entry should be lowercase")

    # Description format: should be capitalized
    if desc and desc[0].islower():
        errors.append(f"  ⚠ {name} binding[{index}]: description should start with uppercase")

    # Duplicate detection
    sig = (key, mods)
    if sig in seen:
        errors.append(f"  ❌ {name} binding[{index}]: duplicate {key} with mods {list(mods)}")
    seen.add(sig)
